...
public sealed class RemoveByIdMainRoleAndUserRLCommandUnitTest
{
    ...
    {
        _serviceMock.Setup(
            x=> x.GetByIdAsync(It.IsAny<string>(),false)).ReturnsAsync(new MainRoleAndUserRelationship());
    }

    [Fact]
    public async Task MainRoleAndUserRelationshipCommandShouldNotBeNull()
    {
